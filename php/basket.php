...
// function that will find the id of the customer and look for the cartxww
function find_id()
{
  $collection = collect_find('Carts');
  // seeing if the customer is logged in
...
// checking if the function returns something, if it does not it won't enter 
if (find_id()) {
  echo '
<!---------------------------- basket layout------------------->
<div class="main">
      ...
      <script id="shopping-cart--list-item-template" type="text/template">
        <li class="_grid shopping-cart--list-item">
          <div class="_column product-image">
            <img class="product-image--img" src="{{=img_url}}" alt="Item image" />
          </div>
...
';
  ...
  // if it does not find anyone in session, it will enter in here
  ...
  <h3 id="h3_basket" >You need to login</h3>
  <p id="p_basket">If you are not logged in you cannot add items to the basket</p>
</div>';
  // if the user is logged in but has nothing in the cart, it will enter in this one
} else {
  ...
  <h3 id="h3_basket" >EMPTY BAG!</h3>
  <p id="p_basket">You will need to add something to your basket first</p>
  </div>';
}
scripts();
outputFooter();
